...
static Scanner sc=new Scanner(System.in);
	...
	public static void customerOperations() {
		CustomerService cdd=new CustomerServiceImpl();
		...
		while(true) {
			...
			case 1:
				...
				try {
					if(cdd.addCustomer(cd)){
						System.out.println("\nCUSTOMER ADDED SUCCESSFULLY !!\n");
					}
					else {
						System.err.println("\nCUSTOMER ID THAT YOU HAVE ENTERED, ALREADY EXISTS !!\n");
					}
				}
			...
			case 2:
				//View all customer detail
				 try {
					...
					if(!result.isEmpty()) {
						System.out.println("\n-----------CUSTOMER DETAILS----------");
						for (CustomerDetailsDto cx : result) {
			...
			case 3:
				...
				cd.setCustomerId(Integer.parseInt(dc12));
				
				try {
					CustomerDetailsDto cDetails=cdd.viewSingleCustomerDetails(cd);
					if(cDetails!=null) {
			...
			case 4:
				...
				try {
					if(cdd.deleteCustomerDetails(cd)) {
			...
			case 5:
				...
				case 1:
					...
					try {
						if(cdd.modifyCustomerDetails(cd, opt)) {
