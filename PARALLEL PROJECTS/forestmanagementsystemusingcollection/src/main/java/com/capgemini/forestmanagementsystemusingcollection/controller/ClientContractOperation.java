...
public class ClientContractOperation extends AdminContractOperations {
//	static ContractService contractService=new ContractServiceImpl();
	...
	public static void clientContractOperation() {
		...
		while (true) {
			...
			System.out.println("Press 2 to view a particular Contract Detail");
			System.out.println("Press 3 to delete a Contract");
			System.out.println("Press 4 to Modify a Contract Detail");
			System.out.println("Press 5 to go Client Page");
			System.out.println("\nEnter your Chice: --> ");
			char a = 'n';
			String choice = null;
			while (a == 'n') {
			...
			case 1:
				...
				TreeMap<Integer, CustomerDetail> checkForCustomerMap=CustomerDaoImpl.mapForCustomer;
				char check='t';
				while(check=='t') {
					if(!checkForCustomerMap.containsKey(Integer.parseInt(customerIdInput))) {
						System.err.println("\nThe customer id that you have entered does`nt exist in the database..\n");
						System.out.println("Enter the proper Customer Id: ");
						char customerIdChoice1 = 'n';
						while (customerIdChoice1 == 'n') {
							customerIdInput = scanner.next();
							if (Validations.isNumber1(customerIdInput)) {
								customerIdChoice1 = 'x';
							} else {
					...
					contractDetails.setCustomerId(Integer.parseInt(customerIdInput));
					check='x';
					}
				}
				// 3.Product Id:
				...
				TreeMap<Integer, ProductDetails> checkForProductMap=ProductDaoImpl.mapForProduct;
				char checkForProduct='y';
				while(checkForProduct=='y') {
					if(!checkForProductMap.containsKey(Integer.parseInt(productIdInput))) {
						System.err.println("\nThe product id that you have entered does`nt exist in the database..\n");
						System.out.println("Enter a proper Product Id: ");
						char productIdChoice1 = 'n';
						while (productIdChoice1 == 'n') {
							productIdInput = scanner.next();
							if (Validations.isNumber1(productIdInput)) {
								productIdChoice1 = 'x';
							} else {
						...
						contractDetails.setProductId(Integer.parseInt(productIdInput));	
						checkForProduct='n';
					}
				}
				// 5. Delivery Date:
